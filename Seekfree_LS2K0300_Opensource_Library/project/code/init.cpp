...
void Encoder_update()
{
    ...
    //转速 = (脉冲数 / 编码器线数) / 采样时间
    encoder_L.count_now = encoder_dir_2.get_count()*2.0;
    encoder_R.count_now = -encoder_dir_1.get_count()*2.0;
...
//    if(it_time%20==0)
//     printf("G_dis:%f,icm_data.gyro_z:%f,icm_data.yaw:%f\n",G_dis,icm_data.gyro_z,icm_data.yaw);
    
    Dis_Speed = LPF_Speed* (Tpm_Dis)+(1-LPF_Speed)*G_dis;//差速
    // LPF_1(1,5.0e-3,Dis_Speed,&Dis_Speed);
    last_G_dis = G_dis;

}
// PWM1=右电机, PWM2=左电机 (已验证)
...
{   
    ...
    if(pwm_l>0)//左电机正转 -> PWM2
    {
        drv8701e_dir_2.set_level(1); 
        drv8701e_pwm_2.set_duty(pwm_l);
    }
    else if(pwm_l<=0)
    {
    ...
    if(pwm_r>0)//右电机正转 -> PWM1
    {
        drv8701e_dir_1.set_level(1);
        drv8701e_pwm_1.set_duty(pwm_r);
    }
    else if(pwm_r<=0)
    {
