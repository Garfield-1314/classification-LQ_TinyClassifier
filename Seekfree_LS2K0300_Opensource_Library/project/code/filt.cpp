...
{
        imu660ra_acc_x = -imu_dev.get_acc_z();// 芯片Z(后)取反→车体前
        imu660ra_acc_y = -imu_dev.get_acc_x();// 芯片X(左)取反→车体右
        imu660ra_acc_z = -imu_dev.get_acc_y();// 芯片Y(下)取反→车体上
...
{
        imu660ra_gyro_x = -imu_dev.get_gyro_z();// 芯片Z(后)取反→绕车体前轴
        imu660ra_gyro_y = -imu_dev.get_gyro_x();// 芯片X(左)取反→绕车体右轴
        imu660ra_gyro_z = -imu_dev.get_gyro_y();// 芯片Y(下)取反→绕车体上轴
...
{
        imu963ra_acc_x = imu_dev.get_acc_x();
        imu963ra_acc_y = imu_dev.get_acc_y();
        imu963ra_acc_z = imu_dev.get_acc_z();
}

void imu963ra_get_gyro(void)
{
        imu963ra_gyro_x = imu_dev.get_gyro_x();
        imu963ra_gyro_y = imu_dev.get_gyro_y();
        imu963ra_gyro_z = imu_dev.get_gyro_z();
}

void imu963ra_get_mag(void)
{
        imu963ra_mag_x = imu_dev.get_mag_x();
        imu963ra_mag_y = imu_dev.get_mag_y();
        imu963ra_mag_z = imu_dev.get_mag_z();
}
...
void gyroOffset_init(void)
{
    ...
    for (uint16_t i = 0; i < 1000; ++i) {  
        // if (imu_dev.get_dev_type() == DEV_IMU963RA) {
        //     imu963ra_get_gyro();
        ...
        system_delay_ms(1);  // 1ms间隔
    }
    
    GyroOffset.Xdata /= 1000.0f;
    GyroOffset.Ydata /= 1000.0f;
    GyroOffset.Zdata /= 1000.0f;
    
    GyroOffset_init = 1;
    
    printf("陀螺仪校准完成:\n");
    printf("  零偏原始值: X=%.1f, Y=%.1f, Z=%.1f\n", 
           GyroOffset.Xdata, GyroOffset.Ydata, GyroOffset.Zdata);
    printf("  零偏物理值: X=%.3f, Y=%.3f, Z=%.3f °/s\n", 
           GyroOffset.Xdata/16.4f, GyroOffset.Ydata/16.4f, GyroOffset.Zdata/16.4f);
}
...
{
    float acc_x_raw = (float)imu963ra_acc_x / 4096.0f;
    float acc_y_raw = (float)imu963ra_acc_y / 4096.0f;
    float acc_z_raw = (float)imu963ra_acc_z / 4096.0f;

    icm_data.acc_x = acc_x_raw * alpha + icm_data.acc_x * (1.0f - alpha);
    icm_data.acc_y = acc_y_raw * alpha + icm_data.acc_y * (1.0f - alpha);
    icm_data.acc_z = acc_z_raw * alpha + icm_data.acc_z * (1.0f - alpha);

    icm_data.gyro_x = ((float) imu963ra_gyro_x - GyroOffset.Xdata) * (M_PI / 180.0f) / 16.4f;
    icm_data.gyro_y = ((float) imu963ra_gyro_y - GyroOffset.Ydata) * (M_PI / 180.0f) / 16.4f;
    icm_data.gyro_z = ((float) imu963ra_gyro_z - GyroOffset.Zdata) * (M_PI / 180.0f) / 16.4f;

    icm_data.mag_x = (float)imu963ra_mag_x;
    icm_data.mag_y = (float)imu963ra_mag_y;
    icm_data.mag_z = (float)imu963ra_mag_z;
}
...
void ICM_AHRSupdate_9axis(float gx, float gy, float gz,
    ...
    if ((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f)) {
        ICM_AHRSupdate(gx, gy, gz, ax, ay, az);
        return;
    }
    ...
    Q_info.q0 = q0 * recipNorm;
    Q_info.q1 = q1 * recipNorm;
    Q_info.q2 = q2 * recipNorm;
    Q_info.q3 = q3 * recipNorm;
}
void ICM_getEulerianAngles(void)
{
    ...
    // if(it_time%20==0)
    //  printf("icm_data.pitch:%f,icm_data.yaw:%f,icm_data.roll :%f\n",icm_data.pitch,icm_data.yaw,icm_data.roll);
}
void ICM_getEulerianAngles_9axis(void) {
    ...
    ICM_getValues_9axis();
    ICM_AHRSupdate_9axis(icm_data.gyro_x, icm_data.gyro_y, icm_data.gyro_z,
                         icm_data.acc_x, icm_data.acc_y, icm_data.acc_z,
                         icm_data.mag_x, icm_data.mag_y, icm_data.mag_z);
    ...
    icm_data.pitch = asinf(-2.0f * q1 * q3 + 2.0f * q0 * q2) * 180.0f / M_PI;
    icm_data.roll = atan2f(2.0f * q2 * q3 + 2.0f * q0 * q1,
                          -2.0f * q1 * q1 - 2.0f * q2 * q2 + 1.0f) * 180.0f / M_PI;
    icm_data.yaw = atan2f(2.0f * q1 * q2 + 2.0f * q0 * q3,
                         -2.0f * q2 * q2 - 2.0f * q3 * q3 + 1.0f) * 180.0f / M_PI;

    if (icm_data.yaw > 180.0f) icm_data.yaw -= 360.0f;
    if (icm_data.yaw < -180.0f) icm_data.yaw += 360.0f;
          it_time++;
   
    if(it_time%20==0)
     printf("imu963ra_mag_x:%f,imu963ra_mag_y:%f,imu963ra_mag_z:%f,icm_data.yaw:%f,gyro_y:%f\n",icm_data.mag_x,icm_data.mag_y,icm_data.mag_z,icm_data.yaw,icm_data.gyro_y);
}
